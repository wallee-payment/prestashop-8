 ...
 */
class WalleeServiceLineitem extends WalleeServiceAbstract
{
    ...
    public function getItemsFromCart(Cart $cart)
    {
        ...
        foreach ($summary['products'] as $productItem) {
            ...
            $name = empty($productItem['name']) ? WalleeHelper::getModuleInstance()->l(
                'Product',
                'lineitem'
            ...
            if ($productItem['id_product'] == Configuration::get(WalleeBasemodule::CK_FEE_ITEM) ||
                $productItem['id_product'] == Configuration::get(WalleeBasemodule::CK_SURCHARGE_ITEM)) {
        ...
        if ($shippingCosts > 0) {
            ...
            $name = empty($name) ? WalleeHelper::getModuleInstance()->l('Shipping', 'lineitem') : $name;
            $name = $this->trimStringByLength($name, 150);
            $item->setName($name);
        ...
        if ($wrappingCosts > 0) {
            ...
            $name = WalleeHelper::getModuleInstance()->l('Wrapping Fee', 'lineitem');
            $name = $this->trimStringByLength($name, 150);
            $item->setName($name);
            $item->setQuantity(1);
            $item->setShippingRequired(false);
        ...
        if (count($summary['discounts']) > 0) {
            ...
            foreach ($summary['discounts'] as $discount) {
                $nameBase = empty($discount['description']) ? WalleeHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
        ...
        if ($discountOnly && $shippingItem != null) {
            ...
            $name = empty($discountOnly['name']) ? WalleeHelper::getModuleInstance()->l(
                'Shipping Discount',
                'lineitem'
            ) : $discountOnly['name'];
            $name = $this->trimStringByLength($name, 150);
    ...
    public function getItemsFromOrders(array $orders)
    {
        ...
        $cleaned = WalleeHelper::cleanupLineItems(
            $items,
            $orderTotal,
            WalleeHelper::convertCurrencyIdToCode($order->id_currency)
        );
        return $cleaned;
    }
    ...
    protected function getItemsFromOrdersInner(array $orders, $orderTotal)
    {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($order->getProducts() as $orderItem) {
                ...
                $name = empty($orderItem['product_name']) ? WalleeHelper::getModuleInstance()->l(
                    'Product',
                    'lineitem'
                ...
                if ($orderItem['product_id'] == Configuration::get(WalleeBasemodule::CK_FEE_ITEM) ||
                    $orderItem['product_id'] == Configuration::get(WalleeBasemodule::CK_SURCHARGE_ITEM)) {
                    $item->setType(\Wallee\Sdk\Model\LineItemType::FEE);
                    $item->setShippingRequired(false);
                }
            ...
            foreach ($order->getCartRules() as $orderCartRule) {
                ...
                $nameBase = empty($orderCartRule['name']) ? WalleeHelper::getModuleInstance()->l(
                    'Discount',
                    'lineitem'
                ) : $orderCartRule['name'];
                $discountItems = $this->getDiscountItems(
