...
class WalleeModelMethodconfiguration extends ObjectModel
{
    ...
    public $fee_base = WalleeBasemodule::TOTAL_MODE_BOTH_INC;
    ...
    {
        $this->title = WalleeTools::base64Encode(serialize($title));
    }

    public function getTitle()
    {
        $decoded = WalleeTools::base64Decode($this->title, true);
        if ($decoded === false) {
            $decoded = $this->title;
        }
    ...
    {
        $this->description = WalleeTools::base64Encode(serialize($description));
    }

    public function getDescription()
    {
        $decoded = WalleeTools::base64Decode($this->description, true);
        if ($decoded === false) {
     ...
     * @param int $id
     * @param int $shopId
     * @return WalleeModelMethodconfiguration | false
     */
    public static function loadByIdWithChecks($id, $shopId)
    {
        ...
        $collection = new PrestaShopCollection('WalleeModelMethodconfiguration');
    ...
    public static function loadByConfigurationAndShop($spaceId, $configurationId, $shopId)
    {
        ...
        if ($result === false) {
            $result = new WalleeModelMethodconfiguration();
        }
        return $result;
    }
