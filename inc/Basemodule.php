...
class WalleeBasemodule
{
    ...
    public static function handleSaveCartRecreation(Wallee $module)
    {
        $output = "";
     ...
     * @return string
     */
    public static function handleSaveCronSettings(Wallee $module)
    {
        $output = "";
        if (Tools::isSubmit('submit' . $module->name . '_email')) {
            if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
    ...
    public static function getCartRecreationConfigValues(Wallee $module)
    {
        $values = array();
        if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
     ...
     * @return mixed[]
     */
    public static function getCronSettingsForm(Wallee $module)
    {
        $cronSettings = array(
            array(
                'type' => 'text',
     ...
     * @return mixed[]
     */
    public static function getCronSettingsConfigValues(Wallee $module)
    {
        $values = array();
        if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
    ...
    public static function hookActionOrderGridDefinitionModifier(Wallee $module, $params)
    {
        ...
        $actionsCollection->add(
            (new LinkRowAction('download_invoice'))
                ->setName("Download Wallee Invoice")
                ->setIcon('description')
                ->setOptions([
    ...
    public static function hookActionAdminControllerSetMedia(Wallee $module, $arr)
    {
        if (Tools::strtolower(Tools::getValue('controller')) == 'adminorders') {
            Media::addJsDefL('wallee_admin_token', $module->getContext()->link->getAdminLink('AdminWalleeDocuments'));
